...
public class LibraryService {
  ...
  public Result borrowBook(String bookId, String memberId) {
    ...
    // First code didnt get check if the book was already loaned out, so i added a check.
    if (entity.getLoanedTo() != null) {
      return Result.failure("ALREADY_LOANED");
    }

    // Fixing the reservation logic
    if (!entity.getReservationQueue().isEmpty()) {
      String firstReserver = entity.getReservationQueue().get(0);
      if (!firstReserver.equals(memberId)) {
        return Result.failure("This book is reserved by another person");
      }
      // Remove from queue since they're now borrowing the said book
  ...
  public ResultWithNext returnBook(String bookId, String memberId) {
    ...
    // If book is not loaned, cannot return it
    if (entity.getLoanedTo() == null) {
      return ResultWithNext.failure();
    }

    // verifying that the member returning the book is the one who has it loaned
    if (memberId != null && !memberId.equals(entity.getLoanedTo())) {
      return ResultWithNext.failure();
    }

    // Find the next eligible member in the reservation queue
    String nextMember = null;
    while (!entity.getReservationQueue().isEmpty()) {
      String candidate = entity.getReservationQueue().remove(0);

      // Check if member still exists and can borrow
      if (memberRepository.existsById(candidate) && canMemberBorrow(candidate)) {
    ...
    if (nextMember != null) {
      // Automatically loan to the next eligible person
      entity.setLoanedTo(nextMember);
      entity.setDueDate(LocalDate.now().plusDays(DEFAULT_LOAN_DAYS));
    } else {
      // checking if there is no next member, then just clear the loan info and make the book
      // avalilable
  ...
  public Result reserveBook(String bookId, String memberId) {
    ...
    // check if member already has the book loaned
    ...
    // If book is available, loan it automatically
    if (entity.getLoanedTo() == null) {
      if (!canMemberBorrow(memberId)) {
        return Result.failure("BORROW_LIMIT");
      }
    ...
    // if book is loaned, add it to the reservation queue
